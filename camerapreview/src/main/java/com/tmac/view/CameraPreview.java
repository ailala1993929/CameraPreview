...
public class CameraPreview extends TextureView {
    ...
    private static final int SAVE_PIC_WIDTH = 1280;//保存图片的宽
    private static final int SAVE_PIC_HEIGHT = 720;//保存图片的高
    ...
    private HandlerThread mExitThread;
...
//            openCamera(this.getWidth(), this.getHeight());
    ...
    private PreviewStatusInterface mListener;

    public void setPreViewStatusListener(PreviewStatusInterface listener) {
        mListener = listener;
    }
    ...
    private boolean isCloseAF;
    private boolean isCloseAE;

    public void setAEMode(boolean close3A) {
        isCloseAE = close3A;
        isCloseAF = close3A;
    }
    ...
    public void setAutoFlash(CaptureRequest.Builder requestBuilder) {
        ...
        Log.d(TAG, "isCloseAE = " + isCloseAE);
        if (isCloseAE) {
            Log.d(TAG, "close AE ..... ");
            requestBuilder.set(CaptureRequest.CONTROL_AE_MODE,
                    CaptureRequest.CONTROL_AE_MODE_OFF);
            requestBuilder.set(CaptureRequest.CONTROL_AE_LOCK,
                    true);
    ...
    private void startBackgroundThread() {
        ...
        mExitThread = new HandlerThread("mExitThread");
        mExitThread.start();
        exitHandler = new ExitHandler(mExitThread.getLooper());
    }

    private void stopBackgroundThread() {
        mBackgroundThread.quitSafely();
        mExitThread.quitSafely();
        try {
    ...
    private CameraCaptureSession.CaptureCallback mCaptureCallback = new CameraCaptureSession.CaptureCallback() {
        ...
        private void process(CaptureResult result) {
            switch (mState) {
                ...
                case STATE_WAITING_LOCK: {//锁定焦点
                    Integer afState = result.get(CaptureResult.CONTROL_AF_STATE);
                    Log.d(TAG + "-Tmac", "afState = " + afState);
                    /**
                            ...
                            CaptureResult.CONTROL_AF_STATE_NOT_FOCUSED_LOCKED == afState)
                    {
                        ...
                        Log.d(TAG + "-Tmac", "aeState = " + aeState);
                        /**
                ...
                case STATE_WAITING_PRE_CAPTURE: {//拍照中
                    Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
                    if (aeState == null ||
                ...
                case STATE_WAITING_NON_PRE_CAPTURE: {//其他
                    Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
        ...
        public void onCaptureCompleted(@NonNull CameraCaptureSession session,
            ...
            Log.d(TAG, "Camera " + mCameraId + " preview data Callback ............... ");
            process(result);
        }
        ...
        public void onCaptureFailed(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, @NonNull CaptureFailure failure) {
            ...
            if (mListener != null) {
                mListener.onPreviewFailed(mCameraId, failure);
            }
        }
    };
...
//            postDelayTakeRGBPhoto();
            //postDelayTakeFisheyePhoto();
            //exitHandler.sendEmptyMessageDelayed(0, 8000);
    ...
    private class ExitHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                ...
                case 1:
                    isSaved = true;
                    if (mListener != null && isCaptureCompleted) {
                        mListener.onPictureSaveSuccess(mCameraID, true);
                    }
                    break;
                case 2:
                    isCaptureCompleted = true;
                    if (mListener != null && isSaved) {
                        mListener.onPictureSaveSuccess(mCameraID, true);
                    }
                    break;
            }
    ...
    private void postDelayTakeRGBPhoto() {
        postDelayed(new Runnable() {
            @Override
            public void run() {
                if (mCameraID.equals("0")) {
                    File rgbFile = new File(getContext().getExternalFilesDir("rgb"), "rgbPic.jpg");
                    Log.e(TAG, "run: take photo for rgb: file path: " + rgbFile.toString());
                    setOutPutDir(rgbFile);
                    takePicture();
                }
            }
        }, 1000);
    ...
    private void postDelayTakeFisheyePhoto() {
        postDelayed(new Runnable() {
            @Override
            public void run() {
                if (mCameraID.equals("1")) {
                    File fisheyeFile = new File(getContext().getExternalFilesDir("fisheye"), "fisheyePic.jpg");
                    Log.e(TAG, "run: take photo for ey: file path: " + fisheyeFile.toString());
                    setOutPutDir(fisheyeFile);
                    takePicture();
                }
            }
        }, 1000);
    ...
    private boolean isContainCameraID;
    ...
    private void setUpCameraOutputs(int width, int height) {
        CameraManager manager = (CameraManager) getContext().getSystemService(Context.CAMERA_SERVICE);
        try {
            Log.d(TAG, "cameraId Size = " + manager.getCameraIdList().length);
            for (String id : manager.getCameraIdList()) {
                Log.d(TAG, "cameraId = " + id);
                if (id.equals(mCameraID)) {
                    isContainCameraID = true;
                }
            }
            ...
            for (String cameraId : manager.getCameraIdList()) {
                ...
                Log.e(TAG, "largest.getWidth():" + largest.getWidth());
                Log.e(TAG, "largest.getHeight():" + largest.getHeight());
                ...
                //预览大小
                Log.e(TAG, "预览大小mPreviewSize: " + mPreviewSize);
    ...
    private void captureStillPicture() {
        Log.d(TAG + "-Tmac", "captureStillPicture ...... ");
        try {
            ...
            CameraCaptureSession.CaptureCallback captureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                    ...
                    Log.d(TAG + "-Tmac", "capture success Saved : " + mFile);
                    unlockFocus();
                ...
                public void onCaptureFailed(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, @NonNull CaptureFailure failure) {
                    super.onCaptureFailed(session, request, failure);
                    //Toast.makeText(getContext(), "capture failed = " + failure, Toast.LENGTH_SHORT).show();
                    Log.d(TAG + "-Tmac", "capture failed = " + failure);
                }
            };
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            if (mCameraID.equals("1")) {
                Log.d(TAG,mCameraID);
                //裁剪图片宽度1280->>>640
                Log.d(TAG,"mFlie= "+mFile);
                mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), mFile));
                Log.d(TAG,"mFile.getAbsoluteFile()= "+mFile.getAbsoluteFile()+" ="+mFile.isFile());
                /*if (!mFile.isFile())
                {
                    Log.d(TAG,"the file has not saved,post delay crop picture!!!!");
                    postDelayCrop();
                }
                else
                {
                    Bitmap bitmap = cropBitmap(mFile);
                    File CropFile = saveBitmapFile(bitmap);
                    Log.d(TAG, "CropFile= " + CropFile);
                }*/

            }else {
                Log.d(TAG,"onImageAvailable======rgb");
                mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), mFile));
            }
    ...
    private void postDelayCrop() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                Log.d(TAG,"postDelayCrop");
                Bitmap bitmap = cropBitmap(mFile);
                File CropFile = saveBitmapFile(bitmap);
                Log.d(TAG, "CropFile= " + CropFile);
            }
    ...
    private File saveBitmapFile(Bitmap bitmap) {
        ...
        Log.d(TAG,"bitmapW="+bitmap.getWidth()+" bitmapH="+bitmap.getHeight());
        File file = new File(getContext().getExternalFilesDir("fisheye"), "fisheyeCropPic.jpg");//将要保存图片的路径
        try {
            BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(file));
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, bos);
            bos.flush();
            bos.close();
            Log.d(TAG,"crop picture success Saved!!!");
        } catch (IOException e) {
            e.printStackTrace();
        }
        return file;
     ...
     * @return
     */
    private Bitmap cropBitmap(File file){
...
//            bitmap = MediaStore.Images.Media.getBitmap(getContext().getContentResolver(), uri);
        ...
        Log.d(TAG,"sourceW= "+ w +" sourceH= "+ h +" cropWidth= "+cropWidth);
